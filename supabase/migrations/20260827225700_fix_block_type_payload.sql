    ...
    for v_submodule in select * from jsonb_array_elements(v_module->'submodules') loop
      ...
      for v_lesson in select * from jsonb_array_elements(v_submodule->'lessons') loop
        ...
        if v_lesson ? 'video_url' and v_lesson->>'video_url' != '' then
          insert into content_blocks (lesson_id, block_type, payload, position)
          values (v_lesson_id, 'video', jsonb_build_object('youtube_url', v_lesson->>'video_url'), 1);
          
          if v_lesson ? 'text_content' and v_lesson->>'text_content' != '' then
            insert into content_blocks (lesson_id, block_type, payload, position)
            values (v_lesson_id, 'text', jsonb_build_object('content_markdown', v_lesson->>'text_content'), 2);
          end if;
        else
          insert into content_blocks (lesson_id, block_type, payload, position)
          values (v_lesson_id, 'text', jsonb_build_object('content_markdown', coalesce(v_lesson->>'text_content', 'Lesson content coming soon.')), 1);
        end if;
    ...
    for v_lesson in select * from jsonb_array_elements(v_submodule->'lessons') loop
      ...
      if jsonb_typeof(v_lesson->'contentBlocks') = 'array' then
        ...
        select 
          ...
          elem->>'type',
          case 
            when elem->>'type' = 'video' then jsonb_build_object('youtube_url', elem->>'content')
            when elem->>'type' = 'text' then jsonb_build_object('content_markdown', elem->>'content')
            else jsonb_build_object('content', elem->>'content')
          end,
