...
begin
  ...
  if p_received_tx_id is not null then
    ...
    if not found then
      raise exception 'Transaction already consumed or not found';
    end if;
  end if;
  ...
  if v_pv.installment_id is not null then
    ...
    -- Always mark order as completed so they get access and aren't swept by abandoned-checkout
    ...
    if p_create_installment and v_shortfall > 0 then
      ...
      -- Create the pending installment for the shortfall safely using interval multiplication
