...
begin
  ...
  if v_pv.installment_id is not null then
    ...
    if p_create_installment and v_shortfall > 0 then
      -- Do NOT mark order as completed, it stays pending since there are dues.
      
      -- Find the primary order item (just pick the first course/bundle)
      select * into v_item from order_items where order_id = v_pv.order_id limit 1;
      
      -- Mark item as using installments
      update order_items set uses_installments = true where id = v_item.id;
      
      -- Create the pending installment for the shortfall
      insert into installments (order_item_id, installment_number, amount, due_at, status)
      values (v_item.id, 2, v_shortfall, now() + (p_due_days || ' days')::interval, 'pending');
      
    else
      -- Forgive balance or paid in full -> complete the order
